...
public class ProductRepository extends FirebaseConnection {
    ...
    public List<String> getReviewsOfProduct(String productId) throws ExecutionException, InterruptedException {
        QuerySnapshot querySnapshot = db.collection("products").document(productId).collection("comments").get().get();
    ...
    public void newCategoryCheck(ArrayList<String> category) throws ExecutionException, InterruptedException {
    ...
    public String addProduct(String description, ArrayList<String> category, double price, String name, String author, String language, int stock, int page, String photoLink) throws IOException, ExecutionException, InterruptedException {
    ...
    public String updateProduct(UpdateProductInputDTO product, Map<String, Object> data) throws JsonProcessingException, ExecutionException, InterruptedException {
    ...
    public String updateProductImage(String productId, String photoLink) throws ExecutionException, InterruptedException, IOException {
    ...
    public boolean isExistsInUserBookmark(String productId, String userId) throws ExecutionException, InterruptedException {
        QuerySnapshot queryBookmarkSnapshot = db.collection("users").whereArrayContains("bookmarks", productId).whereEqualTo(FieldPath.documentId(), userId).get().get();
    ...
    public List<CommentOutputDTO> getProductComments(String productId) throws ExecutionException, InterruptedException {
        ...
        List<QueryDocumentSnapshot> commentDocuments = db.collection("products").document(productId).collection("comments").get().get().getDocuments();
        ...
        for (QueryDocumentSnapshot document : commentDocuments) {
            ...
            newComment.setName(db.collection("users").document(document.getString("userId")).get().get().getString("name"));
    ...
    public List<ProductListingOutputDTO> bestSellerTen() throws ExecutionException, InterruptedException {
        ...
        for (QueryDocumentSnapshot order : orders) {
            List<QueryDocumentSnapshot> products = order.getReference().collection("products").get().get().getDocuments();
        ...
        for (Map.Entry<String, Integer> entry : productCount.entrySet()) {
            if (count != 10) {
                ...
                List<QueryDocumentSnapshot> prices = document.getReference().collection("price").get().get().getDocuments();
    ...
    public List<ProductListingOutputDTO> weeklyDealsTen() throws ExecutionException, InterruptedException {
        ...
        List<QueryDocumentSnapshot> weeklyProducts = db.collection("products").whereEqualTo("weeklyDeal", true).get().get().getDocuments();
        List<ProductListingOutputDTO> returnProducts = new ArrayList<>();
    ...
    public List<ProductListingOutputDTO> getRelationalProducts(String productId) throws ExecutionException, InterruptedException {
            ...
            if (order.getReference().collection("products").document(productId).get().get().exists()) {
                ...
                for (QueryDocumentSnapshot document : products) {
                    if (!document.getId().equals(productId)) {
                        DocumentSnapshot productDocument = db.collection("products").document(document.getId()).get().get();
                        ProductListingOutputDTO productListingOutputDTO = new ProductListingOutputDTO();
                        ...
                        List<QueryDocumentSnapshot> prices = productDocument.getReference().collection("price").get().get().getDocuments();
                        productListingOutputDTO.setPrice(prices.get(prices.size() - 1).getDouble("price"));
