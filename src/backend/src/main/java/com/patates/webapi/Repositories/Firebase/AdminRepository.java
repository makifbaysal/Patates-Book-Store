...
public class AdminRepository extends FirebaseConnection {
    ...
    public int getTotalUsers() throws ExecutionException, InterruptedException {
        QuerySnapshot querySnapshot = db.collection("users").whereEqualTo("active", true).whereEqualTo("isAdmin", false).get().get();
    ...
    public int getTotalOrders() throws ExecutionException, InterruptedException {
        QuerySnapshot querySnapshotOrders = db.collection("orders").whereEqualTo("state", OrderEnum.Shipped.getValue()).get().get();
    ...
    public AdminReportOutputDTO getAdminReports() throws ExecutionException, InterruptedException {
        ...
        ApiFuture<QuerySnapshot> query = db.collection("orders").whereEqualTo("state", OrderEnum.Shipped.getValue()).get();
        ...
        for (QueryDocumentSnapshot document : documents) {
            ...
            DocumentSnapshot queryShippingCompany = db.collection("shippingCompanies").document(cargoId).get().get();
            ...
            List<QueryDocumentSnapshot> documentsPrice = document.getReference().collection("products").get().get().getDocuments();

            for (QueryDocumentSnapshot documentPrice : documentsPrice) {
                DocumentSnapshot documentSnapshot = db.collection("products").document(documentPrice.getId()).get().get();
    ...
    public String addCouponCode(CreateCouponInputDTO createCouponInputDTO) throws ExecutionException, InterruptedException, ParseException {
        DateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy");
        DocumentReference docRef = db.collection("couponCodes").document();
        List<QueryDocumentSnapshot> documents = db.collection("couponCodes").whereEqualTo("state", true).whereEqualTo("code", createCouponInputDTO.getCode()).get().get().getDocuments();
        if (documents.isEmpty()) {
            ...
            docRef.set(data);
            return "Patates'te sepetinizde " + createCouponInputDTO.getLowerLimit() + "₺ ve üzeri alışverişlerinizde geçerli" + " %" + createCouponInputDTO.getPercentageDiscount().intValue() + " indirim şansını yakalamak için \"" + createCouponInputDTO.getCode() + "\" kodunu " + dateFormat.format(createCouponInputDTO.getStartTime()) + " - " + dateFormat.format(createCouponInputDTO.getExpireTime()) + " tarihleri arasında kullanabilirsiniz.";
        } else {
            return "Same code";
        }
    }
    ...
    public List<ListingCouponsOutputDTO> getCouponCodes(String search) throws ExecutionException, InterruptedException {
        ...
        if (search.equals("")) {
            documents = db.collection("couponCodes").whereEqualTo("state", true).get().get().getDocuments();
        } else {
            documents = db.collection("couponCodes").whereEqualTo("state", true).whereEqualTo("code", search).get().get().getDocuments();
        }
    ...
    public CouponCodeDetailsDTO getCouponCodeDetails(String couponId) throws ExecutionException, InterruptedException {

        DocumentSnapshot document = db.collection("couponCodes").document(couponId).get().get();
        return new CouponCodeDetailsDTO(document.getId(), document.getString("header"), document.getString("code"),
                document.getString("description"), document.getLong("remainingQuantity").intValue(), document.getLong("startQuantity").intValue(),
                document.getDouble("lowerLimit"), document.getDouble("percentageDiscount"), document.getDate("expireTime"),
                document.getDate("startTime"));

    }

    public String confirmOrder(AcceptOrderInputDTO acceptOrderInputDTO) throws ExecutionException, InterruptedException {
        try {
            ApiFuture<String> futureTransaction = db.runTransaction(transaction -> {
                ...
                List<QueryDocumentSnapshot> products = orderReference.collection("products").get().get().getDocuments();

                for (QueryDocumentSnapshot product : products) {
                    DocumentReference productReference = db.collection("products").document(product.getId());
                    data.clear();
                    data = new HashMap<>();
                    if (product.getLong("quantity").intValue() > productReference.get().get().getLong("stock").intValue()) {
                        throw new Error();
                    }
                    data.put("stock", productReference.get().get().getLong("stock").intValue() - product.getLong("quantity").intValue());
    ...
    public ArrayList<String> rejectOrder(RejectOrderInputDTO rejectOrderInputDTO) throws ExecutionException, InterruptedException {

        DocumentReference orderReference = db.collection("orders").document(rejectOrderInputDTO.getOrderId());
    ...
    public ArrayList<String> refundOrder(RefundOrderAdminInputDTO refundOrderAdminInputDTO) throws ExecutionException, InterruptedException {
        DocumentReference orderReference = db.collection("orders").document(refundOrderAdminInputDTO.getOrderId());
        Map<String, Object> data = new HashMap<>();
        ArrayList<String> returnList = new ArrayList<>();
    ...
    public List<SeeMessageOutputDTO> seeMessages() throws ExecutionException, InterruptedException {
        ...
        for (QueryDocumentSnapshot document : documents) {
            ...
            seeMessageOutputDTO.setEmail(db.collection("users").document(document.getString("userId")).get().get().getString("email"));
    ...
    public List<String> firebaseElasticDifferenceTest() throws ExecutionException, InterruptedException {
