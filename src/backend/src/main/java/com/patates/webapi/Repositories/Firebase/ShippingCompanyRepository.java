...
public class ShippingCompanyRepository extends FirebaseConnection {
  ...
  public String getFileName(String id) throws ExecutionException, InterruptedException {
    ApiFuture<DocumentSnapshot> queryProduct =
        db.collection("shippingCompanies").document(id).get();
  ...
  public List<ListingShippingCompaniesOutputDTO> getShippingCompanies(String search)
    ...
    if (search.equals("")) {
      documents =
          db.collection("shippingCompanies").whereEqualTo("state", true).get().get().getDocuments();
    } else {
      documents =
    ...
    for (QueryDocumentSnapshot document : documents) {
      listOfShippingCompanies.add(
          new ListingShippingCompaniesOutputDTO(
              document.getId(),
  ...
  public ShippingCompanyDetailsOutputDTO getShippingCompanyDetails(String companyId)
    ...
    return new ShippingCompanyDetailsOutputDTO(
        document.getId(),
        document.getString("image-url"),
        document.getString("name"),
        document.getDouble("price"),
