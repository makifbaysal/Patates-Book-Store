...
public class UserRepository extends FirebaseConnection {
  ...
  public UserDetailsOutputDTO getUserDetail(String id)
  ...
  public String addToBookmark(String userId, String productId)
      throws ExecutionException, InterruptedException {
    ApiFuture<DocumentSnapshot> queryUser = db.collection("users").document(userId).get();
    DocumentSnapshot documentSnapshot = queryUser.get();
  ...
  public String deleteFromBookmark(String userId, String productId)
      throws ExecutionException, InterruptedException {
    ApiFuture<DocumentSnapshot> queryUser = db.collection("users").document(userId).get();
    DocumentSnapshot documentSnapshot = queryUser.get();
  ...
  public List<ProductListingOutputDTO> getBookmark(String userId)
    ...
    QuerySnapshot queryBookmarkSnapshot =
        db.collection("users").whereEqualTo(FieldPath.documentId(), userId).get().get();
    for (QueryDocumentSnapshot document : queryBookmarkSnapshot.getDocuments()) {
      for (String productId : (ArrayList<String>) document.get("bookmarks")) {
        ...
        DocumentSnapshot documentSnapshot =
            db.collection("products").document(productId).get().get();

        QuerySnapshot queryPrice =
            db.collection("products").document(productId).collection("price").get().get();

        returnList.add(
            new ProductListingOutputDTO(
                productId,
                documentSnapshot.getString("image-url"),
                documentSnapshot.getString("name"),
                documentSnapshot.getString("author"),
                queryPrice
                    .getDocuments()
                    .get(queryPrice.getDocuments().size() - 1)
                    .getDouble("price")));
      }
    }
    return returnList;
  ...
  public String addToShoppingCart(AddToShoppingCartInputDTO addToShoppingCartInputDTO)
    ...
    DocumentReference docRef =
        db.collection("users").document(addToShoppingCartInputDTO.getUserId());

    DocumentReference docRefShoppingCart =
        docRef.collection("shoppingCart").document(addToShoppingCartInputDTO.getProductId());
  ...
  public List<ShoppingCartProductListingOutputDTO> getShoppingCart(String userId)
    ...
    List<QueryDocumentSnapshot> documents =
        docRef.collection("shoppingCart").get().get().getDocuments();
    for (QueryDocumentSnapshot document : documents) {
      ...
      shoppingCartProductList.add(
          new ShoppingCartProductListingOutputDTO(
              productId,
              productDocument.getString("image-url"),
              productDocument.getString("name"),
              productDocument.getString("author"),
              priceList.get(priceList.size() - 1).getDouble("price"),
  ...
  public String updateShoppingCart(UpdateShoppingCartInputDTO updateShoppingCartInputDTO)
      throws ExecutionException, InterruptedException {
    DocumentReference docRef =
        db.collection("users").document(updateShoppingCartInputDTO.getUserId());
    DocumentReference shoppingCartReference =
        docRef.collection("shoppingCart").document(updateShoppingCartInputDTO.getProductId());

    Map<String, Object> data = new HashMap<>();
    data.put(
        "quantity",
        shoppingCartReference.get().get().getDouble("quantity").intValue()
            + updateShoppingCartInputDTO.getValue());
  ...
  public String deleteProductShoppingCart(
    ...
    DocumentReference docRef =
        db.collection("users").document(deleteProductShoppingCartInputDTO.getUserId());

    DocumentReference shoppingCartReference =
        docRef
            .collection("shoppingCart")
            .document(deleteProductShoppingCartInputDTO.getProductId());
  ...
  public String updateAddress(UpdateAddressInputDTO updateAddressInputDTO) {
    ...
    DocumentReference docRefaddress =
        docRef.collection("addresses").document(updateAddressInputDTO.getAddressId());
  ...
  public String deleteAddress(DeleteAddressInputDTO deleteAddressInputDTO) {
    ...
    DocumentReference docRefaddress =
        docRef.collection("addresses").document(deleteAddressInputDTO.getAddressId());
  ...
  public List<ListAddressOutputDTO> getAddresses(String userId)
    ...
    List<QueryDocumentSnapshot> documentsList =
        docRef.collection("addresses").get().get().getDocuments();

    for (QueryDocumentSnapshot document : documentsList) {
      listAddress.add(
          new ListAddressOutputDTO(
              document.getId(),
              document.getString("header"),
              document.getString("address"),
              document.getString("city"),
              document.getString("county")));
    }
    return listAddress;
  }

  public AddressDetailsOutputDTO addressDetails(String userId, String addressId)
      throws ExecutionException, InterruptedException {
    DocumentReference docRef = db.collection("users").document(userId);

    DocumentSnapshot document = docRef.collection("addresses").document(addressId).get().get();

    return new AddressDetailsOutputDTO(
        addressId,
        document.getString("header"),
        document.getString("address"),
        document.getString("city"),
  ...
  public String creditCartExists(String cartNumber, String owner, String date, String cvc)
      throws ExecutionException, InterruptedException {
    List<QueryDocumentSnapshot> queryDocumentSnapshots =
        db.collection("creditCarts")
            .whereEqualTo("cartNumber", cartNumber)
  ...
  public String updateCreditCart(
      UpdateCreditCartInputDTO updateCreditCartInputDTO, String cartStatus)
      throws ExecutionException, InterruptedException {
    ApiFuture<String> futureTransaction =
        db.runTransaction(
            transaction -> {
              DocumentReference docRef =
                  db.collection("users").document(updateCreditCartInputDTO.getUserId());

              DocumentReference docRefCart =
                  docRef.collection("creditCarts").document(updateCreditCartInputDTO.getCartId());

              transaction.delete(docRefCart);

              DocumentReference docRefNewCart =
                  docRef.collection("creditCarts").document(cartStatus);
              Map<String, Object> data = new HashMap<>();
  ...
  public String deleteCreditCart(DeleteCreditCartInputDTO deleteCreditCartInputDTO) {
    DocumentReference docRef =
        db.collection("users").document(deleteCreditCartInputDTO.getUserId());

    DocumentReference docRefCart =
        docRef.collection("creditCarts").document(deleteCreditCartInputDTO.getCartId());
  ...
  public List<ListCreditCartsOutputDTO> getCreditCarts(String userId)
    ...
    List<QueryDocumentSnapshot> documentsList =
        docRef.collection("creditCarts").get().get().getDocuments();

    for (QueryDocumentSnapshot document : documentsList) {
      listCarts.add(
          new ListCreditCartsOutputDTO(
              document.getId(),
              document.getString("cartNumber"),
  ...
  public CreditCartDetailsOutputDTO creditCartDetails(String userId, String cartId)
    ...
    return new CreditCartDetailsOutputDTO(
        userId,
        cartId,
        document.getString("owner"),
        document.getString("cartNumber"),
  ...
  public String makeComment(@RequestBody MakeCommentInputDTO makeCommentInputDTO) {
    ...
    DocumentReference docProductRef =
        db.collection("products")
            .document(makeCommentInputDTO.getProductId())
            .collection("comments")
            .document();
    Map<String, Object> data = new HashMap<>();
  ...
  public List<UserCommentOutputDTO> getUserComments(String userId)
    ...
    List<QueryDocumentSnapshot> productDocuments =
        db.collection("products").get().get().getDocuments();

    for (QueryDocumentSnapshot product : productDocuments) {
      List<QueryDocumentSnapshot> commentDocuments =
          product.getReference().collection("comments").get().get().getDocuments();
      ...
      for (QueryDocumentSnapshot comment : commentDocuments) {
        if (comment.getString("userId").equals(userId)) {
          ...
          userComment.setName(
              db.collection("users").document(userId).get().get().getString("name"));
  ...
  public List<ListingCouponsOutputDTO> getAvailableCouponCodes()
    ...
    for (QueryDocumentSnapshot document : documents) {
      if (document.getDate("startTime").compareTo(new Date()) != 1
          && document.getDate("expireTime").compareTo(new Date()) != -1
          && document.getLong("remainingQuantity").intValue() > 0) {
  ...
  public UseCouponCodeOutputDTO useCouponCode(String code)
    ...
    if (documents.isEmpty()
        || documents.get(0).getLong("remainingQuantity").intValue() <= 0
        || documents.get(0).getDate("expireTime").compareTo(new Date()) != 1
        || documents.get(0).getDate("startTime").compareTo(new Date()) != -1) {
      return null;
    } else {
      return new UseCouponCodeOutputDTO(
          documents.get(0).getDouble("lowerLimit"),
          documents.get(0).getDouble("percentageDiscount"));
    }
  }
  ...
  public String deleteComment(DeleteCommentInputDTO deleteCommentInputDTO) {
    DocumentReference docProductRef =
        db.collection("products")
            .document(deleteCommentInputDTO.getProductId())
            .collection("comments")
            .document(deleteCommentInputDTO.getCommentId());
  ...
  public String updateComment(UpdateCommentInputDTO updateCommentInputDTO)
    ...
    DocumentReference docProductRef =
        db.collection("products")
            .document(updateCommentInputDTO.getProductId())
            .collection("comments")
            .document(updateCommentInputDTO.getCommentId());
    Map<String, Object> data = new HashMap<>();
  ...
  public CommentDetailsOutputDTO commentDetail(CommentDetailsInputDTO commentDetailsInputDTO)
    ...
    DocumentReference commentRef =
        db.collection("products")
            .document(commentDetailsInputDTO.getProductId())
            .collection("comments")
            .document(commentDetailsInputDTO.getCommentId());
  ...
  public boolean accountExists(String email) throws ExecutionException, InterruptedException {
    List<QueryDocumentSnapshot> user =
        db.collection("users").whereEqualTo("email", email).get().get().getDocuments();
    return !user.isEmpty();
  }
}
